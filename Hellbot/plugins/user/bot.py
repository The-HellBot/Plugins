...
from Hellbot.core import hellbot, db, ENV
...
async def ping(_, message: Message):
    ...
    if img:
        await message.reply_document(
            img,
            caption=PING_TEXT.format(speed, uptime, message.from_user.mention),
            force_document=False,
        )
        return
    await hellbot.edit(hell, PING_TEXT.format(speed, uptime, message.from_user.mention))


HelpMenu("bot").add(
    "alive", None, "Get the alive message of the bot.", "alive"
).add(
    "ping", None, "Check the ping speed and uptime of bot.", "ping", "You can also customize ping message by adding a media to it."
).info(
    "Alive Menu"
).done()
